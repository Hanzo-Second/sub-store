...
async function deleteServer(name, id) { if (id) { if (!confirm(`Delete ${name}?`)) return; try { await api(`/api/proxies/${id}`, { method: 'DELETE' }); await refreshWorkspace(); } catch (error) { alert(error.message); } return; } servers = servers.filter((server) => server.name !== name); localStorage.setItem('substore-servers', JSON.stringify(servers)); renderServers($('#server-search')?.value || ''); }
...
function renderAccessKeys() {
  ...
  if (!rows) return;
  rows.innerHTML = state.accessKeys.length ? state.accessKeys.map((key) => { const url = key.key ? subscriptionURL(key.key) : `${(state.settings?.baseUrl || location.origin).replace(/\/$/, '')}/sub/${key.keyPreview}`; const copy = key.key ? `<button class="icon-action" data-copy-access="${key.id}" aria-label="Copy URL for ${escapeHTML(key.name)}" title="Copy subscription URL">${copyIcon}</button>` : ''; const allowance = key.monthlyDataGB > 0 ? `${Number(key.monthlyDataGB).toLocaleString()} GB/month` : 'Unlimited'; return `<div class="access-key-row"><div class="key-value"><div class="key-icon">⌘</div><div><strong>${escapeHTML(key.name)}</strong><span>${escapeHTML(url)}</span></div></div><span>Private subscription</span><span class="status-pill ${key.enabled ? 'online' : 'offline'}"><i></i>${key.enabled ? 'Active' : 'Disabled'}</span><span class="muted-text">${escapeHTML(allowance)}</span><span class="muted-text">${key.lastUsedAt ? escapeHTML(key.lastUsedAt) : 'Never'}</span><span class="row-actions">${copy}<button class="icon-action" data-edit-access="${key.id}" aria-label="Edit ${escapeHTML(key.name)}" title="Edit access key">${editIcon}</button><button class="icon-action danger" data-delete-access="${key.id}" aria-label="Delete ${escapeHTML(key.name)}" title="Delete access key">${trashIcon}</button></span></div>`; }).join('') : '<div class="empty-preview"><div class="empty-icon">⌘</div><h3>No access keys yet</h3><p>Create one to share your generated configuration.</p></div>';
  ...
  $$('[data-delete-access]').forEach((button) => button.addEventListener('click', async () => { if (!confirm('Delete this access key?')) return; try { await api(`/api/access-keys/${button.dataset.deleteAccess}`, { method: 'DELETE' }); await refreshWorkspace(); } catch (error) { alert(error.message); } }));
...
document.addEventListener('submit', async (event) => {
  ...
  if (id) {
    ...
    try { const result = await api(`/api/access-keys/${id}`, { method: 'PATCH', body: JSON.stringify({ name: data.keyName, enabled: data.enabled === 'on', monthlyDataGB: Number(data.monthlyDataGB || 0), ...(key ? { key } : {}) }) }); closeAccessModal(); await refreshWorkspace(); if (result.key) showAccessKeyResult(result.key, 'Replacement key ready'); else { showToast(); $('.toast strong').textContent = 'Access key updated'; $('.toast p').textContent = 'The access key changes are saved.'; } } catch (error) { alert(error.message); }
    return;
  }
  const manual = $('.key-mode.active').dataset.keyMode === 'manual';
  const key = manual ? data.manualKey : data.generatedKey;
  try { const result = await api('/api/access-keys', { method: 'POST', body: JSON.stringify({ name: data.keyName, key, monthlyDataGB: Number(data.monthlyDataGB || 0) }) }); closeAccessModal(); await refreshWorkspace(); showAccessKeyResult(result.key, 'Access key ready'); } catch (error) { alert(error.message); }
}, true);
...
function groupMemberChoices(type, editingGroupID = '') {
  if (type === 'subscription') return (state.subscriptions || []).filter((item) => item.enabled).map((item) => ({ value: `subscription-id:${item.id}`, label: item.name }));
  if (type === 'group') return (state.groups || []).filter((item) => item.enabled && String(item.id) !== String(editingGroupID)).map((item) => ({ value: `group-id:${item.id}`, label: item.name }));
  if (type === 'proxy') return servers.filter((item) => Number(item.id) > 0 && item.enabled).map((item) => ({ value: item.name, label: item.name }));
  return [{ value: 'DIRECT', label: 'DIRECT' }];
}
function updateGroupMemberValue(row, selectedValue = '') {
  ...
  const choices = groupMemberChoices(type, editingID);
...
function addGroupMember(member = null) {
  ...
  row.innerHTML = '<select data-member-type aria-label="Member type"><option value="subscription">Subscription</option><option value="group">Proxy group</option><option value="proxy">Manual proxy</option><option value="direct">DIRECT</option></select><select data-member-value aria-label="Member"></select><button type="button" class="group-member-remove" aria-label="Remove member" title="Remove member">×</button>';
  const typeSelect = row.querySelector('[data-member-type]');
  typeSelect.value = parsed.type;
  updateGroupMemberValue(row, parsed.value);
  typeSelect.addEventListener('change', () => updateGroupMemberValue(row));
  row.querySelector('.group-member-remove').addEventListener('click', () => row.remove());
...
function renderSubscriptions(items) {
  ...
  if (!grid) return;
  const cheap = items.find((item) => item.name.includes('光喵'));
  const share = $('.config-share'); if (share) { let usage = $('#combined-usage'); if (!usage) { usage = document.createElement('p'); usage.id = 'combined-usage'; usage.className = 'field-hint'; share.querySelector('.share-url')?.after(usage); } usage.textContent = cheap?.totalGB ? `Combined subscription usage mirrors Cheap: ${Number(cheap.usedGB || 0).toFixed(2)} / ${Number(cheap.totalGB).toFixed(2)} GB` : 'Combined subscription usage mirrors provider-reported Cheap usage. Update that subscription to refresh it.'; }
  ...
  $$('[data-sub-delete]').forEach((button) => button.addEventListener('click', async () => { if (!confirm('Delete this subscription?')) return; try { await api(`/api/subscriptions/${button.dataset.subDelete}`, { method: 'DELETE' }); await refreshWorkspace(); } catch (error) { alert(error.message); } }));
...
function renderRules(items) {
  ...
  $$('[data-rule-delete]').forEach((button) => button.addEventListener('click', async () => { if (!confirm('Delete this routing rule?')) return; try { await api(`/api/rules/${button.dataset.ruleDelete}`, { method: 'DELETE' }); await refreshWorkspace(); } catch (error) { alert(error.message); } }));
...
function renderServiceRuleGroups(items) {
  ...
  $$('[data-delete-service-group]').forEach((button) => button.addEventListener('click', async () => { if (!confirm('Delete this service rule group and remove all of its inline rules from generated config?')) return; try { await api(`/api/service-rule-groups/${button.dataset.deleteServiceGroup}`, { method: 'DELETE' }); await refreshWorkspace(); } catch (error) { alert(error.message); } }));
}
...
function renderGroups(items) {
  ...
  $$('[data-group-delete]').forEach((button) => button.addEventListener('click', async () => { if (!confirm('Delete this proxy group?')) return; try { await api(`/api/groups/${button.dataset.groupDelete}`, { method: 'DELETE' }); await refreshWorkspace(); } catch (error) { alert(error.message); } }));
}
...
function renderRuleProviders(items) {
  ...
  $$('[data-provider-delete]').forEach((button) => button.addEventListener('click', async () => { if (!confirm('Delete this rule provider?')) return; try { await api(`/api/rule-providers/${button.dataset.providerDelete}`, { method: 'DELETE' }); await refreshWorkspace(); } catch (error) { alert(error.message); } }));
  $('[data-add-provider]')?.addEventListener('click', () => openProviderModal());
}

async function refreshConfig() { try { const yaml = await api('/api/config/preview'); const pre = $('#view-config pre'); if (pre) pre.textContent = yaml; } catch (error) { console.warn(error); } }
function showNotice(title, message) { $('.toast strong').textContent = title; $('.toast p').textContent = message; showToast(); }
function renderClientConfig(settings) { const form = $('#client-config-form'); if (!form) return; if (!form.elements.groupLayout) { const section = document.createElement('section'); section.className = 'client-config-section'; section.innerHTML = '<h2>Display</h2><p>This changes the web interface only; it does not change generated YAML.</p><label>Proxy group layout<select name="groupLayout"><option value="cards">Cards</option><option value="rows">One line</option></select></label>'; form.append(section); } Object.entries(settings).forEach(([key, value]) => { const field = form.elements[key]; if (!field) return; field.type === 'checkbox' ? field.checked = value : field.value = value; }); form.elements.groupLayout.value = localStorage.getItem('substore-group-layout') || 'cards'; const rows = [['Mixed port', settings.mixedPort], ['Allow LAN', settings.allowLan ? 'Enabled' : 'Disabled'], ['Mode', settings.mode], ['DNS', settings.dnsEnabled ? `${settings.dnsEnhancedMode} enabled` : 'Disabled'], ['Nameservers', settings.dnsNameservers], ['Fallback', settings.dnsFallback]]; $('#client-config-summary').innerHTML = rows.map(([name, value]) => `<div><dt>${escapeHTML(name)}</dt><dd title="${escapeHTML(value)}">${escapeHTML(value)}</dd></div>`).join(''); }
